...
public:
    ...
    vector<float> box_depth;
    vector<cv::Point2d> box_velocity;
    vector<bool> omit;
    ...
    std::vector<std::vector<cv::KeyPoint>> mvdynKeys, mvdynKeysUn;
    std::vector<cv::KeyPoint> mvKeysUn;

    // Corresponding stereo coordinate and depth for each keypoint.
    // "Monocular" keypoints have a negative value.
    std::vector<float> mvuRight, mvDepth;
    std::vector<std::vector<float>> mvudynRight, mvdynDepth;
    ...
    std::vector<cv::Mat> mdynDescriptors;
...
private:
    ...
    void boxTrack(vector<cv::Rect2d> &boxes, Frame &last_frame);
    bool firstSeparate(const cv::Mat &mask, vector<cv::Rect2d> &boxes, vector<vector<int>> &index, vector<bool> &hasKpts);
