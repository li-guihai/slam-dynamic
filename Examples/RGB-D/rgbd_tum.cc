...
int main(int argc, char **argv)
{
    if(argc != 5 && argc != 6 )
    {
        cerr << endl << "Usage: ./rgbd_tum path_to_vocabulary path_to_settings path_to_sequence path_to_association (yolo)" << endl;
        return 1;
    }
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        if(argc == 6 ) {
            ...
            boxes = yolo->Segmentation_(imRGB);
            ...
            cout<<"img_"<<ni<<" detect time: "<<ttrack;

            if(boxes.empty())
                cout<<"No Target."<<endl;

            SLAM.TrackRGBD(imRGB,imD,boxes, tframe);    
