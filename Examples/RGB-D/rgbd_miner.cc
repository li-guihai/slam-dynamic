...
void LoadMinerImages(const string &strPathToSequence, vector<string> &vstrImageLeft,
                vector<string> &vstrImageDepth, vector<string> &masks,
                vector<vector<Rect2d>> &boxesAll, vector<double> &vTimestamps);
...
int main(int argc, char **argv)
{
    ...
    LoadMinerImages(argv[3], vstrImageFilenamesRGB, vstrImageFilenamesD, masks, boxesAll, vTimestamps);
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        imD = cv::imread(vstrImageFilenamesD[ni],CV_LOAD_IMAGE_GRAYSCALE);
        mask = cv::imread(masks[ni],CV_LOAD_IMAGE_GRAYSCALE);
                ...
                vector<vector<Rect2d>> &boxesAll, vector<double> &vTimestamps)
{
    struct dirent *ptr;    
    DIR *dir;
    string rgbPath = strPathToSequence+"/original";
    dir=opendir(rgbPath.c_str()); 
    vector<string> files;
    ...
    int nTimes = files.size();     
    boxesAll.resize(nTimes);
    for(int i=0; i<nTimes-1; i++)
    {
        stringstream ss;
        ss << i+1;
        vstrImageLeft.push_back(strPathToSequence + "/original/" + ss.str() + ".jpg");
        vstrImageDepth.push_back(strPathToSequence + "/range/" + ss.str() + ".jpg");
        masks.push_back(strPathToSequence + "/mask/" + ss.str() + ".jpg");

        ifstream box_info;
        string name = strPathToSequence + "/bbox/" + ss.str() + ".txt";
        box_info.open(name.c_str());
        if(!box_info.good())
            continue;
        vector<Rect2d> boxes;
        while (!box_info.eof())
        {
            ...
            if(!box_.empty())
            {
                stringstream ss_box;
                ss_box << box_;
                double id, center_x, center_y, width, height; //之前搞错了，第一个是minx
                ss_box >> id >> center_x >> center_y >> width >> height;
                cv::Rect2d temp(center_x, MAX(center_y - height / 2, 0), width, height);
                boxes.push_back(temp);
            }
        } 
        boxesAll[i] = boxes;
        vTimestamps.push_back(i/5.0);
    }
}
